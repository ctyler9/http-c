...
static_assert(IMHTTP_RES_META_CAPACITY <= IMHTTP_RES_BODY_CHUNK_CAPACITY,
              "If we overshoot and read part of body into meta data buffer, we "
              "want that \"tail\" to actually fit into the \"res_body_chunk\"");
...
void imhttp_req_body_chunk_sized(ImHTTP *imhttp, const char *chunk,
                                 size_t chunk_size);
...
bool imhttp_res_next_header(ImHTTP *imhttp, String_View *name,
                            String_View *value);
...
static void imhttp_write_cstr(ImHTTP *imhttp, const char *cstr) {
  ...
  // TODO: imhttp_write doesn't handle imHTTP_Write errors
...
void imhttp_res_begin(ImHTTP *imhttp) {
  ...
  {
    ssize_t n = imhttp->read(imhttp->socket, imhttp->res_meta,
                             IMHTTP_RES_META_CAPACITY);
    // TODO: imhttp_res_being does not handle read errors
    assert(n > 0);
    imhttp->res_meta_size = n;
    imhttp_res_springback_meta(imhttp);

    imhttp->meta_cursor =
        (String_View){.count = imhttp->res_meta_size, .data = imhttp->res_meta};
  };
}
...
bool imhttp_res_next_header(ImHTTP *imhttp, String_View *name,
  ...
  if (!sv_eq(line, SV("\r"))) {
    // TODO: don't set name/value if the user set to NULL
...
bool imhttp_res_next_body_chunk(ImHTTP *imhttp, String_View *chunk) {
  ...
  if (imhttp->content_length > 0) {
    if (imhttp->first_chunk) {
      ...
      ssize_t n = imhttp->read(imhttp->socket, imhttp->res_body_chunk,
                               IMHTTP_RES_BODY_CHUNK_CAPACITY);
      assert(n > 0);
      imhttp->res_body_chunk_size = n;
    }
    if (chunk) {
      *chunk = (String_View){.count = imhttp->res_body_chunk_size,
                             .data = imhttp->res_body_chunk};
    }

    // TODO: imHttp does not handle the situation when teh server responeded
    // with more data than it claimed with Content-Length
    assert((int)imhttp->res_body_chunk_size <= imhttp->content_length);
    imhttp->content_length -= imhttp->res_body_chunk_size;
    return true;
  }
  assert(0 && "TODO: imhttp_res_next_body_chunk not implemented");
  return false;
}

void imhttp_res_end(ImHTTP *imhttp) { (void)imhttp; }
